#!/usr/bin/env python3
"""Fresh ngspice qualification of proposed I32 JLC substitution electrical bounds.

No ECAD mutation. Reuses the exact I32 selected transient model/case matrix and
applies proposed replacement component electrical corners.
"""
...
CAND = {
    ...
    "R156": {"mpn":"RN73R1JTTD5051B10", "nom":5050.0, "tol":0.0010, "tcr_ppm":10.0, "drift":0.0010},
    "R160": {"mpn":"RG1608P-6341-W-T5", "nom":6340.0, "tol":0.0005, "tcr_ppm":25.0, "drift":0.0010},
    ...
    "R169": {"mpn":"RG1608V-3011-P-T1", "nom":3010.0, "tol":0.0002, "tcr_ppm":5.0, "drift":0.0010},
    "R170": {"mpn":"PTFR0603Q1K00N9", "nom":1000.0, "tol":0.0002, "tcr_ppm":10.0, "drift":0.0010},
}
DT_HOT_C = 130.0
for v in CAND.values():
    v["total_frac"] = v["tol"] + v["tcr_ppm"]*1e-6*DT_HOT_C + v["drift"]
...
def accepted_summary(results):
    ...
    maxrail=max(v["main_peak_V"] for v in main)
...
def main():
    out=D/"substitution_spice_results"
    ...
    summary.update({
        ...
        "R156_note":"Functional model uses concrete KOA RN73R1JTTD5051B10 electrical limits (5.05k, +/-0.1%, 10ppm/C). Source availability is a separate procurement gate.",
