...
old=W/'iterations/I23_return_clearance/candidate_kicad/GR86_CCA_RevB.kicad_pcb';new=W/'iterations/I24_return_clearance_final/candidate_kicad/GR86_CCA_RevB.kicad_pcb'
a,ai,u=c.collect(old);b,bi,v=c.collect(new);assert not u and not v
j=json.loads((D/'FINAL_CANDIDATE_MANIFEST.json').read_text());removed=set(j['removed_uuids']);aa={c.get(s,'uuid')[0]:s for s in c.child(a,'segment')};bb={c.get(s,'uuid')[0]:s for s in c.child(b,'segment')};added=set(bb)-set(aa)
...
assert {names[c.get(aa[k],'net')[0]]for k in removed}=={'OIL_5V','OIL_SIG','LED_CAN'}
assert {names[c.get(bb[k],'net')[0]]for k in added}=={'OIL_5V','OIL_SIG','LED_CAN'}
...
remaining=json.loads((W/'analyses/review_i23/RETURN_AND_RF_REVIEW.json').read_text())['remaining_segments']
fallback=U([from_wkt(r['adjacent_gap_WKT']).buffer(.15)for r in remaining if r['net']=='CAN_TX_MCU'])
alternate=min(fallback.distance(i['geometry'])for i in bi if i['id']in added and i['layer']=='In2.Cu');assert alternate>=.155-2e-6,alternate
report={'alternate_TX_reference_minimum_projected_clearance_mm':alternate,'six_adjacent_CAN_TX_gaps_remain':True,'status':'PASS_INDEPENDENT_SOURCE_PREFLIGHT_NATIVE_REQUIRED','source_before_sha256':hashlib.sha256(old.read_bytes()).hexdigest(),'source_after_sha256':hashlib.sha256(new.read_bytes()).hexdigest(),'changed_nets':['OIL_5V','OIL_SIG','LED_CAN'],'removed_segments':len(removed),'added_segments':len(added),'all_nonsegment_objects_exact':True,'unchanged_segment_objects_exact':True,'new_clearance_regressions':regress,'lost_explicit_pad_groups':lost,'all_track_keepout_polygons_checked':keepout_count,'new_track_keepout_hits':hits,'voltage_specific_minimum_clearance_mm':mins,'voltage_specific_findings':vh,'projected_adjacent_reference_regressions':projected,'minimum_copper_board_edge_mm':edge,'minimum_copper_NPTH_mm':hole,'native_required':True,'affected_PDN_ground_thermal_required':True,'physical_tests_claimed':0}
(D/'FINAL_SOURCE_CHANGE_CHECK.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(report,indent=2))
